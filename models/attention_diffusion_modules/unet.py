...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        # mid_attention_channels = int(cond_embed_dim/8)
        # layers = [
        #     nn.Conv2d(3, self.inner_channel, 1),
        #     AttentionBlock( self.inner_channel,
        #                     use_checkpoint=use_checkpoint,
        #                     num_heads=1,
        #                     num_head_channels=num_head_channels,
        #                     use_new_attention_order=use_new_attention_order,
        #                     ),
        #     nn.Conv2d(self.inner_channel, mid_attention_channels, 1),
        #     ResBlock(
        #         mid_attention_channels,
        #         cond_embed_dim,
        #         dropout,
        #         out_channel=mid_attention_channels,
        #         use_checkpoint=use_checkpoint,
        #         use_scale_shift_norm=use_scale_shift_norm,
        #     ),
        #     nn.Conv2d(mid_attention_channels, 1, 1),
        # ]
        layers = [
            Downsample(channels=3, use_conv=True, out_channel=8),
            Downsample(channels=8, use_conv=True, out_channel=32),
            ResBlock(
                32,
                cond_embed_dim,
                dropout,
            ...
            Upsample(channels=32, use_conv=True, out_channel=8),
            Upsample(channels=8, use_conv=True, out_channel=3),
            nn.Conv2d(3, 1, 1)
        ]
        self.attention_block = EmbedSequential(*layers)

        ch = input_ch = int(channel_mults[0] * inner_channel)
        self.input_blocks = nn.ModuleList(
            [EmbedSequential(nn.Conv2d(in_channel, ch, 3, padding=1))]
        )
        self.mid_attention_blocks = nn.ModuleList([])
        ...
        for level, mult in list(enumerate(channel_mults))[::-1]:
            for i in range(res_blocks + 1):
                ...
                self._feature_size += ch
                # self.mid_attention_blocks.append(
                #         AttentionBlock(
                #             ich,
                #             use_checkpoint=use_checkpoint,
                #             num_heads=1,
                ...
                #             use_new_attention_order=use_new_attention_order,
                #         ))
                self.mid_attention_blocks.append(nn.Conv2d(ich, ich, 1))
    ...
    def forward(self, x, x_noise, gammas):
        """
        ...
        # c = x
        # c = x[:, 1, :, :].unsqueeze(1)
        c = self.attention_block(x, emb)
        x = torch.cat([c, x, x_noise], dim=1)
        h = x.type(torch.float32)
        for module in self.input_blocks:
            h = module(h, emb)
            hs.append(h)
        h = self.middle_block(h, emb)
        for module, attention_module in zip(self.output_blocks, self.mid_attention_blocks):
            h = torch.cat([h, attention_module(hs.pop())], dim=1)
            h = module(h, emb)
        h = h.type(x.dtype)
